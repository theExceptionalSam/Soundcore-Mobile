...
export function TestimonialsEditor() {
  const { data, setData, loading, saving, dirty, save } = useContent<Testimonial[]>("testimonials");
  ...
  if (!data) return <div className="text-red-400 py-10 text-center">Failed to load testimonials</div>;

  function update(i: number, patch: Partial<Testimonial>) {
    setData((prev) => prev ? prev.map((t, idx) => (idx === i ? { ...t, ...patch } : t)) : prev);
  }
  function add() {
  ...
  function remove(i: number) {
    if (!confirm(`Delete review from "${data?.[i]?.name}"?`)) return;
  ...
  async function handleSave() {
    ...
    const r = await save();
    toast.dismiss(t);
    if (r?.ok) toast.success("Testimonials updated. Live in ~30s.");
  ...
  return (
    <div className="space-y-6">
      <EditorHeader
        title="Testimonials"
        subtitle="Client reviews shown in the carousel on the Reviews page. First review = first slide."
        onSave={handleSave}
        saving={saving}
        dirty={dirty}
      />
      {data.map((t, i) => (
        <Card key={i} className="bg-zinc-900 border-zinc-800">
          <CardContent className="p-5 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_80px] gap-3">
              <div>
                ...
                <Input
                  ...
                  placeholder="Bruno D. (Portugal)"
                />
              </div>
              <div>
                <Label className="text-zinc-400 text-xs">Role / project</Label>
                <Input
                  value={t.role}
              ...
              <div>
                ...
                <Input
                  value={t.initial}
                  onChange={(e) => update(i, { initial: e.target.value.slice(0, 1) })}
                  className="bg-zinc-950 border-zinc-700 text-zinc-100 text-center"
                  maxLength={1}
                />
              </div>
            </div>
            ...
            <div>
              <Label className="text-zinc-400 text-xs">Stars (★★★★★)</Label>
              <Input
                value={t.stars}
